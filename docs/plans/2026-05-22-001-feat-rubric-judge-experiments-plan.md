...
**Dependencies:** U2 (schema must define `rubric_node` and `triangle_verdict_node` before lab-tech can reference them in its readiness output)
...
- **Hash-lock check (R5):** if the experiment was previously run (lab-tech sees a prior readiness report in `docs/research/lab-readiness-*` or the DAG has a `rubric_hash` for this experiment), compare the recorded hash against the current `.nerd/rubrics/<id>.yaml` content hash. Mismatch → `[BLOCKER] rubric_hash_mismatch: rubric "<id>" was hash-locked at signed_at <ts> with hash <old>; current hash is <new>. Copy .nerd/rubrics/<id>.yaml to a new id, edit, and re-run with rubric:<new-id> — there is no in-band amendment.` First-time runs: compute and persist hash, emit `signed_at`.
...
- **Triangle gate (R2):** before running, consult the DAG for a `triangle_verdict_node` matching `(rubric_hash, judge_id)`. **Read path** (honoring the architecture-decisions filtered-markdown rule): the orchestrator pre-loads a filtered-markdown view of triangle verdicts into lab-tech's invocation context — a small block such as `Triangle verdicts on file: (rubric_hash=abc1234..., judge=claude-opus-4-7) PASS 13/15 verified 2026-05-12; (rubric_hash=def5678..., judge=claude-opus-4-7) FAIL 6/15 verified 2026-04-30`. U4's Approach extends report-compiler to emit this filtered triangle-verdict block as part of the per-batch context handoff. Cache hit AND status=PASS AND `verified_at` within `triangle_cache_days` → emit `[OK] triangle cached (verified <date>, <correct>/<total> trials)` and skip. Cache hit AND status=FAIL → `[BLOCKER] judge_fails_triangle_discriminability` (cached). Cache miss or expired → run the triangle: generate N≥10 trials, half {good,good,bad} half {good,bad,bad}, shuffle order, prompt judge per the structural template (see Key Decisions), check ≥80% correct identification (binomial p<0.05).
- **Triangle verdict persistence path:** lab-tech does NOT write the DAG directly (single-writer invariant: only report-compiler and loop-scout write). When a fresh triangle test runs, lab-tech emits the verdict into its readiness report (`docs/research/lab-readiness-*.md`) as a structured block (`triangle_verdict: { rubric_hash: <sha>, judge_id: <id>, correct_count: <n>, total_trials: <n>, status: <PASS|FAIL>, verified_at: <ISO> }`); report-compiler reads the readiness report at batch-end (it already consumes readiness reports for the existing has_harness flow) and writes the `triangle_verdict_node` to the DAG. This keeps lab-tech as a markdown producer and report-compiler as the sole DAG writer.
- **Integration concern (flagged from learnings):** the DAG read path in step 1 (hash-lock) must use the filtered-markdown convention from `docs/solutions/architecture-decisions/research-dag-cross-session-memory.md`, not raw JSON. report-compiler's existing per-experiment summary needs to surface `rubric_hash` and `triangle_verdict_id` in markdown form so lab-tech can consume it on resume. U4 owns this emit.
...
- Error path (hash mismatch): rubric file edited after a prior run. lab-tech reads prior `rubric_hash` from DAG (via report-compiler's markdown summary), compares against current file hash, fails. `[BLOCKER] rubric_hash_mismatch: ...` with both hashes and the fork instruction. Covers AE5.
...
- Error path: a `triangle_verdict_node` with `correct_count > total_trials` fails (cross-field constraint; if JSON Schema can't express it, document as a planning-time assertion that report-compiler must check).
...
**Goal:** Extend `commands/nerd.md` Phase 2c to recognize `instrument: judge_rubric` and route the experiment through U1's judge-instrument gate instead of the existing numeric U5 path (R6). Also wire the Phase 6c launch prose so the rubric (id-or-path) and judge_id are passed forward as invocation parameters to the experiment-executor — the executor's *internal* judge-rubric branch is owned by U6, not this unit.

**Requirements:** R6

**Dependencies:** U1 (the gate must exist for the routing to be meaningful)

**Files:**
- Modify: `commands/nerd.md` (Phase 2c classification, ~lines 247-287; Phase 6c "Launch Experiment Agents" prose at ~lines 366-411 to carry rubric/judge into the executor invocation parameters)
...
- Phase 6c's existing executor invocation pattern (two-phase build/run from commit `0f74c16`) — the rubric and judge_id are added as additional parameters in the executor prompt template, not a new invocation shape.
...
- The executor invocation prose in Phase 6c references rubric and judge for rubric-judged experiments.
...
**Goal:** Extend `agents/report-compiler.md` to emit the rubric-experiment fields on verdict nodes (R4) — `rubric_id`, `rubric_hash`, `judge_id`, `triangle_verdict_id`, `criterion_scores` — and to surface `rubric_hash` in the per-experiment markdown summary that lab-tech consumes on resume (the integration concern flagged in U1).

**Requirements:** R4 (emit logic; structural fields owned by U2)

**Dependencies:** U2 (schema must define the optional fields before report-compiler emits them); soft-deps U1 (the data report-compiler emits comes from lab-tech's gate verdict)

**Files:**
- Modify: `agents/report-compiler.md` (theory-node template ~lines 176-188; the per-experiment markdown summary section that lab-tech reads on resume — surface the `rubric_hash` there in plain text so lab-tech's filtered-markdown read works without DAG JSON parsing)
...
- The per-experiment markdown summary (consumed by lab-tech on resume per the architecture-decisions learning) gains a `Rubric:` block: `Rubric: portrait-v3 (hash: <8-char prefix>...); judge: claude-opus-4-7; triangle: PASS (15/15, verified 2026-05-12)`. This is the filtered-markdown surface U1's hash-lock check reads — keeps lab-tech off the raw DAG JSON.
...
- **Triangle-verdict cache filtered-markdown surface**: when the orchestrator (or `/nerd` Phase 4.5) hands context to lab-tech for a pre-flight, it includes a small block listing currently-on-file triangle verdicts: `Triangle verdicts: (rubric_hash=abc1234..., judge=claude-opus-4-7) PASS 13/15 verified 2026-05-12; ...`. report-compiler is the producer of this block (it owns the DAG → markdown filtered view); the orchestrator passes it through to lab-tech. This is U1's triangle-cache read path.
...
- Integration (round-trip): lab-tech on a subsequent run reads the markdown summary, parses the `Rubric:` line, extracts `rubric_hash`, and compares it against the current `.nerd/rubrics/<id>.yaml` file hash. This is the integration U1's hash-lock check depends on — verify the producer (this unit) and consumer (U1) agree on the line format.
- Integration (triangle-verdict write path): lab-tech writes a `triangle_verdict:` structured block in its readiness report after running a fresh triangle test. report-compiler reads the readiness report at batch-end and emits a `triangle_verdict_node` to the DAG. Subsequent pre-flights see the cached verdict via the filtered-markdown block.
...
**Verification:**
- A rubric experiment's DAG output contains all R4 fields, validates against the U2 schema, and the markdown summary contains a parseable `Rubric:` line.
- A numeric experiment's output is byte-identical to what the existing report-compiler would produce today (no regressions).
- The markdown summary line format is documented in `agents/report-compiler.md` so future readers can parse it without inferring from examples.
...
- The two-phase split (`phase=build` / `phase=run`) collapses to a single phase for judge-rubric experiments — there is no harness to build (no metric command to wire into an eval module). The executor in judge-rubric mode is `phase=run`-only; `phase=build` is a no-op that returns immediately with "no harness needed for judge-rubric mode."
...
- **State lifecycle risks:** The hash-lock semantics (R5) introduce a cross-invocation state dependency — lab-tech must read the prior `rubric_hash` from the DAG to detect mid-experiment drift. This is the only state-across-invocations concern; the U4 design routes the read through report-compiler's markdown summary to honor the DAG-invariants doc's "orchestrator-mediated reads, not raw JSON" rule. Verify both producer and consumer (U4 emit, U1 read) agree on the line format.
...
- **Integration coverage:** The integration concern (lab-tech reading prior `rubric_hash` from the DAG via report-compiler's markdown summary) crosses three units (U4 emits, U1 reads, U2 schema admits the field). Cross-unit verification belongs in U4's Integration test scenarios — confirm the producer/consumer format agreement before declaring U1 done.
...
| Report-compiler's `Rubric:` summary line format diverges between U4 (emit) and U1 (read) | U4's Integration test scenario explicitly round-trips the line format through both sides. Document the format in `agents/report-compiler.md` (not just example-based). |
...
U1, U3, U4, U6 are independent of each other but all depend on U2:
- U1 reads (eventually) from U4's emitted markdown — but only on resume of an experiment that's already run once. First-time landing of U1 and U4 in either order works; the cross-unit integration test (U4's Integration scenario) verifies the format agreement before U1's hash-lock check can fire on real data.
- U3's Phase 2c routing dispatches to U1's gate — U1 must exist for U3 to mean anything operationally, but the units can land in either order if U3 is documented as targeting a gate that will exist when U1 is merged.
