...
## Step 2: Install Autoresearch Training
...
### No GPU
Inform the user: "No GPU detected. LLM training experiments via /autoresearch will be skipped. Codebase parameter experiments will still work."
...
```bash
mkdir -p ~/.claude/plugins/autoresearch
```

Write to `~/.claude/plugins/autoresearch/hardware-profile.yaml`:

```yaml
# Auto-generated by /autoresearch-setup
# Re-run /autoresearch-setup to recalibrate
...
Create `.claude/autoresearch.local.md` if it doesn't exist:
...
Ready! Run /autoresearch to start.
```
